# 15 — Il Vigneto del Mago dei Vini

> *Un vecchio vigneto, druidi invasori, tre gemme magiche, e il segreto dei Custodi delle Piume.*

---

### La Richiesta di Danika

Prima di lasciare [Vallaki](<../../../lore/strahd/#vallaki>), **[Danika](<../../../png/strahd/#danika-martikov>)** ricorda al gruppo la richiesta di andare a recuperare le scorte di vino alla **Wizard of Wines**, il vigneto che rifornisce tutta [Barovia](<../../../lore/strahd/#barovia>). Le scorte stanno terminando.

### Il Viaggio Verso Krezk

Il gruppo lascia [Vallaki](<../../../lore/strahd/#vallaki>) di notte, diretto verso ovest. Durante il viaggio, vengono attaccati da un branco di **lupi mannari**. **[[Slap]]** e **[[Luth]]** vengono morsi. **[[Nezuko]]** protegge Slap e fugge.

### Il Vigneto

Arrivati al vigneto, incontrano il vecchio **Daviani Martikov**. Il vigneto è stato invaso da druidi malvagi con un'orda di creature arboree.

Daviani spiega il segreto del vigneto: **tre gemme magiche** create nell'antichità erano sepolte nel terreno per renderlo fertile. Una è stata rubata dieci anni fa da un certo **[Escher](<../../../png/strahd/#escher>)**. La seconda è stata rubata un anno fa da spaventapasseri provenienti da **[Berez](<../../../lore/strahd/#berez>)**. Ora i druidi cercano la terza. Se la troveranno, il vino di [Barovia](<../../../lore/strahd/#barovia>) sarà perduto per sempre.

### L'Assalto alla Vineria

Il gruppo si reca alla vineria. Centinaia di creature arboree escono dal vigneto e li circondano. Riescono a rifugiarsi all'interno.

Un **gigantesco albero animato** irrompe nel vigneto. Un druido armeggia in un enorme tino di vino. La vineria viene scossa dai potenti colpi dell'albero.

La casa crolla. Alcuni restano sepolti nelle macerie, altri riescono a fuggire.

### La Cantina e il Mimic

Nella cantina, il gruppo scopre un passaggio segreto che porta a una grotta. Uno scrigno si rivela essere un **mimic**. Una creatura di melma e muffa emerge dalle pareti.

### La Vittoria e la Scoperta

In superficie, **[[Laszlo]]** e **[Sarek](<../../../png/strahd/#sarek>)** sconfiggono il druido. Usando il bastone del druido, Laszlo può passare incolume tra le creature arboree.

L'albero gigante emette un grido e richiama tutte le creature. Se ne vanno, portando con sé la terza gemma.

### Il Totem del Corvo

Una volta estratti dalle macerie, **Daviani** racconta la storia dei **[Custodi delle Piume](<../../../fazioni/strahd/#custodi-delle-piume>)**. Il **Totem del Corvo** — l'unico rimasto non corrotto a Barovia — è stato tramandato per generazioni. Conferisce il dono della **korantropia** (licantropi corvo).

Il gruppo recupera del vino e riparte verso **[Krezk](<../../../lore/strahd/#krezk>)**.

---

## NPC Incontrati

- **Daviani Martikov** — Vecchio vignaiolo. Custode del segreto delle gemme e del Totem del Corvo.
- **I Martikov** — La famiglia del vigneto. Discendenti dei Krezkov.

---

## Eventi Chiave

- Il vigneto è invaso dai druidi
- Scoperta delle tre gemme magiche
- Battaglia alla vineria e crollo dell'edificio
- Scoperta del Totem del Corvo
- La storia dei [Custodi delle Piume](<../../../fazioni/strahd/#custodi-delle-piume>)

---

[← **Capitolo precedente**](../14-festival-vallaki/) | [**Capitolo successivo →**](16-argynvostholt/)

*[Torna all'indice](/sessioni/strahd/)*